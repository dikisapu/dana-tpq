...
Tahun : <?php echo $namabulan; ?> <?php echo $tahun; ?>
...
<?php						
	...
	$mySql = "SELECT * FROM spp 
	JOIN santri ON spp.id_santri = santri.id_santri AND  year(tgl_bayar) = '$tahun'
	GROUP BY spp.id_spp 
...
<?php										
	$mySql = "SELECT sum(jmlh_pembayaran) as jmlh_pembayaran FROM spp WHERE  year(tgl_bayar) = '$tahun'";
	$myQry = mysql_query($mySql, $koneksidb)  or die ("Query salah : ".mysql_error());  
	$myData = mysql_fetch_array($myQry);
